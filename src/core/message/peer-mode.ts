 ...
 *   2. 合并后的 chatmode 场景表 `configured`（chatmode.{private|group|nothuman}）——
 *      关系级与 agent 级同名字典，已由 ConfigManager 按 x-merge:dict 逐键合并
 *   3. schema 出厂默认表 `chatmodeFactoryDefaults()`
 *
 * chatmode 是顶层字典参数（agent 级与关系级形态相同），实际生效值由对端类型选键。
 * 详见 config-reference.md §三。
 */
