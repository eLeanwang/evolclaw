...
export class BootstrapService {
  ...
  private resolveConfiguredRecipient(config: AgentConfig, channelKey: string, channelType?: string): string | undefined {
    if (channelType === 'aun') return config.owners?.[0];
    const parsed = this.parseChannelKey(channelKey);
    const inst = parsed
      ? config.channels.find(c => c.type === parsed.type && c.name === parsed.name)
      : config.channels.find(c => c.name === channelKey);
