...
export class DingtalkChannelPlugin implements ChannelPlugin {
  ...
  async createInstance(inst: DingtalkInst, ctx: ChannelBuildContext): Promise<ChannelInstance | null> {
    if (inst.enabled === false) return null;
    if (!isValidCredential(inst.clientId) || !isValidCredential(inst.clientSecret)) return null;
