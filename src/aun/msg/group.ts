...
async function resolvePublishedRulesFile(
  ...
  if (forcePull) {
    try {
      const index = await client.group.getGroupIndex({ group_id: groupId }) as Record<string, unknown>;
...
async function currentGroupIndexEtag(client: AUNClient, groupId: string): Promise<string | undefined> {
  try {
    const check = await client.group.checkGroupIndex({ group_id: groupId }) as Record<string, unknown>;
