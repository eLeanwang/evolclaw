...
async function processAuxiliaryBatch(trigger: TriggerReason) {
  // 1. 提取批次（只取 PENDING，提取即标记 processedByAuxiliary）
  ...
  const input: AuxiliaryInput = {
    ...
    aunMessages: {
      newMessages: batch,
      // 【信号A】去掉本批次后，辅助队列还剩多少条待判断
      remainingInQueue: auxiliaryQueue.size() - batch.length,
    },
    mainSession: {
      status: mainSession.getStatus(),
      // 【信号B】主队列待处理数（不含正在处理的批次）
      pendingCount: mainQueue.size(),
    },
  };
...
async function executeDecision(output: AuxiliaryOutput) {
  ...
  if (action === 'hold') {
    ...
    // 计算延迟时间（单聊与群聊公式相同）
    // 实际延迟 = baseDelayMs + random(0, baseLevelMs(delayLevel) × 对端系数)
    const levelMs = baseLevelMs(delayLevel || 'medium');
    const peerFactor = isAgentPeer(batch) ? 1.0 : 0.5;  // 含 agent→1.0，全是人→0.5
    const totalDelay = config.baseDelayMs + Math.random() * (levelMs * peerFactor);
