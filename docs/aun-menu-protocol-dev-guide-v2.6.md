...
最后更新：2026-07-02
对应代码：`src/types.ts` · `src/core/message/message-bridge.ts` · `src/core/command/command-handler.ts`（门面） · `src/core/command/menu-handler.ts`（menu 执行） · `src/core/message/command-handler-agent-control.ts`（agent 生命周期桥接） · `src/channels/aun.ts` · `src/index.ts`（控制 AID 通道 menu.* / trigger.* 路由）
...
>
> **v2.6 变更（trigger v3 管理）**：`trigger` 从旧的 delay/at/cron 简化触发器扩展为 v3 definition 模型。`menu.options/update/action name=trigger` 支持启用/停用、删除、立即执行、脚本执行、event trigger、三分支 feedback、并发/错过策略、限制参数；同时补充 daemon/ECWeb 使用的 IPC `trigger.list/show/eventCatalog/create/update/setEnabled/cancel/delete/run`。详见 §6.8、§8.7。
...
| `trigger` | — | — | ✅ | ✅ | `set` `enable` `disable` `delete` `run` `cancel` | 关系级 | v3 触发器管理；scoped 可见性，见 §8.7 |
...
MENU_NAME_MAP = {
  ...
  trigger:    '/trigger',   // 关系级，自主触发器；高级详情/事件目录走 trigger.* IPC，见 §8.7
...
`menu.options name=trigger` 返回当前 agent 下可见 trigger 的菜单摘要。触发器是**关系级 scoped 资源**：
- admin/owner 可见全部 trigger
- guest 仅可见自己创建的 trigger（`definition.origin.peerId === userId` 且 `definition.origin.channel === channel`）
- 无 trigger scheduler 时返回空数组 `[]`

`args.options` 控制范围：

| args.options | 返回范围 |
|---|---|
| 省略 / `enabled` | 当前启用的 trigger |
| `all` | 启用 + 停用 trigger |
...
{ "data": [
    {
      "value": "trig_daily_report",
      "label": "每日开发日报",
      "desc": "cron | 下次 2026/7/3 09:30:00",
      "id": "trig_daily_report",
      "name": "每日开发日报",
      "enabled": true,
      "scheduleType": "cron",
      "scheduleValue": "30 9 * * *",
      "nextFireAt": 1783032600000,
      "targetChannel": "aun#bot.agentid.pub#main",
      "targetChannelId": "oc_xxx",
      "targetSessionStrategy": "isolated",
      "status": "active",
...
字段说明：
- `value` / `id` — trigger ID，后续 `menu.update` / `menu.action` 可用作 `nameOrId`
- `label` / `name` — trigger 展示名
- `desc` — `${scheduleType}`，若有 `nextFireAt` 则追加本地化的「下次 ...」
- `status` — `active` / `disabled`
- `subscription.status` — event trigger 的订阅状态：`active` / `inactive` / `event-bus-unavailable` / `not-event`

`menu.options` 是列表摘要，不返回完整 `trigger.json`、脚本预览、active runs 或审计历史。需要详情的客户端应通过 daemon IPC `trigger.show` 查询，见 §8.7 的 “IPC trigger.*” 小节。
...
服务端校验图片后上传到该 agent 自己的 AUN storage，再通过 `agentmdPut()` 重新签名发布 `agent.md`。
这两类 agent.md 更新不得与 config patch 混合提交；未知 patch 字段返回 `UNSUPPORTED_FIELD`。

```jsonc
// →
{ "type": "menu.action", "id": "a-207", "name": "agent", "action": "update",
  "args": {
    "aid": "mybot.agentid.pub",
    "patch": {
...
{ "data": {
    "aid": "mybot.agentid.pub",
    "avatar": "https://mybot.agentid.pub/shared/avatars/uuid.png",
    "saved": true
  }}

// ← avatar 不能与 config patch 混合提交
{ "error": {
    "code": "INVALID_ARGS",
...
- 后端按解码后的图片字节数硬限制 `<= 500 * 1024` bytes。
- 返回的 `avatar` URL 为 AID 风格 URL：`https://{aid}/shared/avatars/{uuid}.{ext}`。
- 成功后 EventBus 会发布 `agent:avatar_updated`，payload 为 `{ aid, avatar, timestamp? }`。

### 8.6 agent create 构建进度（轮询）

create 受理后，后台逐环节把进度写入 `agents/<aid>/create-status.json`，并在 query 响应里以
`createProgress` 透出。`accepted=true` 表示该进度状态已可查询；即使 `config.json` 尚未落盘，
`menu.query name=agent` 也会返回该 AID 的最小占位 payload + `createProgress`，而不是 `NOT_FOUND`。
客户端轮询 `createProgress.status` 直到 `ready` / `failed`。
...
v2.6 起，trigger 的持久化模型是 schema v3 `TriggerDefinition`。Menu Protocol 保留关系级入口，适合聊天客户端做列表、快速创建、编辑和操作；ECWeb/CLI 的详情、脚本预览、事件目录和完整 definition 导入导出走 daemon IPC `trigger.*`，见本节 “IPC trigger.*”。

鉴权走 channel 角色 + scoped：
- admin/owner 可查看、创建、更新、启停、删除、立即执行任意 trigger
- guest 可创建 trigger；后续只能操作自己创建的 trigger
- guest scoped 判定使用 `definition.origin.peerId === userId` 且 `definition.origin.channel === channel`
- 非 admin 且 `userId` 缺失时，更新/启停/删除/立即执行返回 `FORBIDDEN`
- 无 trigger scheduler 时，action/update 返回 `NOT_SUPPORTED`

#### v3 TriggerDefinition 核心结构
...
interface TriggerDefinition {
  ...
  createdAt: number;
  updatedAt: number;
  origin?: { channel?: string; peerId?: string; sessionKey?: string };
  ...
  limits?: { maxRuns?: number; maxDuration?: string };
}
```
...
| `source.type` | 字段 | 说明 |
|---|---|---|
| `delay` | `afterMs` | 创建后延迟触发一次 |
| `at` | `at` | ISO 时间触发一次 |
| `cron` | `expression`, `timezone?` | cron 周期触发；timezone 可选 |
| `interval` | `everyMs` | 固定间隔触发 |
| `event` | `eventPattern`, `filter?` | EventBus 事件触发 |

event pattern 支持 `*`、精确事件名（如 `message:received`）和 namespace 前缀（如 `message:*`）。`filter.match` 使用点路径匹配 event payload：
...
```jsonc
{
  "source": {
    ...
    "filter": {
      "match": {
        "channel": "aun#bot.agentid.pub#main",
        "content": { "$regex": "日报|周报" },
        "timestamp": { "$gte": 1783000000000 },
...
match value 支持标量、`$in`、`$regex`、`$gt/$gte/$lt/$lte`、`$exists`。`filter.where` 不支持。
...
| mode | 必填字段 | 说明 |
|---|---|---|
| `agent` | `execution.prompt` | 把 prompt 注入 agent-runner 执行 |
| `script` | `execution.script.path`, `execution.script.runtime` | 执行 trigger 目录内脚本，脚本 stdout 必须返回 v3 `TriggerReply` |

`execution.script.path` 可以包含脚本参数，例如 `scripts/job.js --foo bar`。服务端会解析第一段为相对脚本路径，其余作为 `script.args`；不再要求 UI 单独提交“脚本入参 JSON”。脚本路径必须留在 trigger 目录内，禁止绝对路径和 `..` 穿越。

`execution.session.strategy` 只接受：

| strategy | Menu UI 名称 | 行为 |
|---|---|---|
| `isolated` | 最新活跃会话 | 触发时按目标 channel/channelId 选择最新可用会话；无历史时创建隔离会话 |
| `thread` | 指定话题会话 | 使用 `threadId` 对应话题会话；未提供时服务端可用 `trigger:<id>` 兜底 |

历史 `current` / 当前活跃会话机制已停用。`menu.update` 提交 `targetSessionStrategy=current` 会返回 `INVALID_ARGS`。

`feedback` 显式分三类，不再支持 `feedback.default`：

| 分支 | 触发条件 |
|---|---|
| `onReply` | agent/script 有正常回复 |
| `onNoop` | 返回 noop |
| `onFailure` | 执行失败、脚本错误或 agent error |

每个分支的 action：

| kind | 字段 | 说明 |
|---|---|---|
| `forward` | `targets[]`, `template?` | 把结果转发到目标 channel/channelId/threadId |
| `reply-origin` | `delivery`, `template?` | 回到触发来源 |
| `silent` | — | 不反馈 |

`delivery` 可为 `direct` 或 `inbound`：`direct` 直接发消息；`inbound` 注入 agent-runner 队列，按普通入站消息处理。

#### menu.action set — 兼容快速创建

`set` 是兼容入口，用结构化 args 组装 trigger，适合聊天客户端快速创建。它不覆盖完整 v3 definition 的所有字段；复杂 trigger 建议走 IPC `trigger.create`。
...
// ←
{ "data": { "id": "trig_daily_report", "name": "每日开发日报", "nextFireAt": 1783032600000 } }
```

当前 menu `set` 校验 `scheduleType` 为 `delay` / `at` / `cron` / `interval`，但实现中的兼容路径仍以传统时间触发为主；event trigger 和脚本 trigger 应使用 `trigger.create` 或先创建后通过 `menu.update` 编辑。

#### menu.update — 编辑 trigger

`menu.update name=trigger` 的 `value` 是 JSON 字符串，必须包含 `nameOrId`。

```jsonc
// →
{ "type": "menu.update", "id": "u-301", "name": "trigger",
  "value": "{\"nameOrId\":\"trig_daily_report\",\"scheduleType\":\"event\",\"scheduleValue\":\"message:*\",\"eventFilter\":{\"match\":{\"content\":{\"$regex\":\"日报\"}}},\"executionMode\":\"script\",\"scriptPath\":\"scripts/report.js --today\",\"scriptRuntime\":\"node\",\"feedback\":{\"onReply\":{\"kind\":\"forward\",\"delivery\":\"direct\"},\"onNoop\":{\"kind\":\"silent\"},\"onFailure\":{\"kind\":\"forward\",\"delivery\":\"inbound\"}},\"concurrency\":\"forbid\",\"missedPolicy\":\"run_once\",\"maxRuns\":30,\"maxDuration\":\"2h\"}" }

// ←
{ "data": { "id": "trig_daily_report", "nextFireAt": 1783032600000 } }
```
...
|---|---|
| `name` | 修改展示名 |
| `scheduleType` / `scheduleValue` | `delay` / `at` / `cron` / `interval` / `event`；event 的 value 是 `eventPattern` |
| `eventFilter` | event trigger 的 `source.filter`；传 `null` / `{}` 清空 |
| `timezone` | cron timezone |
| `executionMode` | `agent` / `script` |
| `prompt` | agent prompt；script 模式下作为 onReply output template |
| `scriptPath` / `scriptRuntime` / `scriptArgs` | script 配置；`scriptPath=null` 或 `scriptRuntime=null` 可回到 agent 模式 |
| `model` / `effort` / `permissionMode` | 本 trigger 执行覆盖；空字符串或 `null` 表示继承 |
| `targetChannel` / `targetChannelId` / `targetSessionStrategy` / `targetThreadId` | 反馈目标和会话选择；不支持跨 agent 迁移 |
| `feedback` | `{ onReply?, onNoop?, onFailure? }`，每项 `{ kind, delivery?, template? }` |
| `mode` | 兼容字段：`direct`/`direct-message` 把所有非 silent 分支 delivery 改为 `direct`；`agent`/`agent-runner` 改为 `inbound` |
| `onFailure` / `onNoop` | 兼容字段，仅 script trigger：`notify` / `silent` |
| `maxRuns` / `maxDuration` | 限制；`maxDuration` 格式 `30s` / `15m` / `2h` / `1d` |
| `concurrency` | `forbid` / `replace` / `allow` |
| `missedPolicy` | `skip` / `run_once` / `run_all` |
...
{ "error": { "code": "INVALID_ARGS", "message": "targetSessionStrategy=current 已停用，请改用 latest 或 thread" } }
```

#### menu.action enable / disable / delete / run / cancel

```jsonc
// 启用
{ "type": "menu.action", "id": "a-302", "name": "trigger", "action": "enable",
  "args": { "nameOrId": "trig_daily_report" } }
// ←
{ "data": { "id": "trig_daily_report", "enabled": true } }

// 停用
{ "type": "menu.action", "id": "a-303", "name": "trigger", "action": "disable",
  "args": { "nameOrId": "trig_daily_report" } }
// ←
{ "data": { "id": "trig_daily_report", "enabled": false } }

// 删除：只允许已停用 trigger
{ "type": "menu.action", "id": "a-304", "name": "trigger", "action": "delete",
  "args": { "nameOrId": "trig_daily_report" } }
// ←
{ "data": { "id": "trig_daily_report", "deleted": true } }

// 立即执行：只允许已启用 trigger
{ "type": "menu.action", "id": "a-305", "name": "trigger", "action": "run",
  "args": { "nameOrId": "trig_daily_report", "dryRun": false } }
// ←
{ "data": { "id": "trig_daily_report", "runId": "run_xxx", "status": "completed", "reason": null } }

// 取消：兼容旧入口，语义是 cancel trigger
{ "type": "menu.action", "id": "a-306", "name": "trigger", "action": "cancel",
  "args": { "nameOrId": "trig_daily_report" } }
// ←
{ "data": { "id": "trig_daily_report", "cancelled": true } }
```

`delete` 对启用 trigger 返回 `INVALID_STATE`。`run` 对停用 trigger 返回 `INVALID_STATE`。

#### IPC trigger.* — ECWeb/CLI 详情与完整管理

这些不是 `menu.*` payload，而是 daemon 本地 IPC command。ECWeb 通过 websocket `ipc` 桥接使用，CLI `ec trigger` 直接使用。

```jsonc
{ "type": "trigger.list", "agentAid": "eleanbot.agentid.pub", "all": true }
// ← { "ok": true, "triggers": [ TriggerDefinition, ... ] }

{ "type": "trigger.show", "agentAid": "eleanbot.agentid.pub", "triggerId": "trig_daily_report" }
// ← { "ok": true, "definition": TriggerDefinition, "active": [], "schedule": {}, "limitState": {}, "recentRuns": [], "scriptPreview": {}, "subscription": {} }

{ "type": "trigger.eventCatalog", "includeInternal": false }
// ← { "ok": true, "events": [ { "type": "message:received", "namespace": "message", "name": "received", "fields": [ ... ] } ] }

{ "type": "trigger.create", "definition": TriggerDefinition, "files": [], "enable": true }
// ← { "ok": true, "trigger": TriggerDefinition }

{ "type": "trigger.update", "agentAid": "eleanbot.agentid.pub", "triggerId": "trig_daily_report",
  "definition": TriggerDefinition, "files": [] }
// ← { "ok": true, "trigger": TriggerDefinition }

{ "type": "trigger.setEnabled", "agentAid": "eleanbot.agentid.pub", "triggerId": "trig_daily_report", "enabled": false }
{ "type": "trigger.cancel", "agentAid": "eleanbot.agentid.pub", "triggerId": "trig_daily_report" }
{ "type": "trigger.delete", "agentAid": "eleanbot.agentid.pub", "triggerId": "trig_daily_report" }
{ "type": "trigger.run", "agentAid": "eleanbot.agentid.pub", "triggerId": "trig_daily_report", "dryRun": true }
```

`trigger.show.subscription.status` 用于可观测性：

| status | 含义 |
|---|---|
| `active` | event trigger 已订阅 EventBus |
| `inactive` | event trigger 未订阅，需刷新 scheduler 或排查注册 |
| `event-bus-unavailable` | 当前 scheduler 未挂 EventBus |
| `not-event` | 非 event trigger |
...
name 速查
  ...
  trigger   options / update / action(*)      触发器列表/编辑/set+enable+disable+delete+run+cancel（关系级 scoped）
