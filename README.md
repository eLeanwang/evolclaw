...
轻量级多终端接力开发工具，支持飞书、微信、AUN 多渠道接入，基于 Claude Agent SDK 构建。
...
- 🌐 **多渠道接入**：飞书 WebSocket + 微信 ClawBot + AUN 协议
...
1. **消息渠道层** (`src/channels/`) - Feishu WebSocket + WeChat HTTP 长轮询 + AUN 协议
...
│   ├── sessions.db             # 会话数据库
│   ├── wechat-sync-buf.txt     # 微信长轮询游标
│   └── wechat-context-tokens.json # 微信 context_token 缓存
└── logs/
    ├── evolclaw.pid            # PID 文件
    ├── evolclaw.log            # 主日志
    ├── stdout.log              # 标准输出
