...
- 🔐 **分层权限**：四级权限体系（guest/user/admin/owner），多用户安全隔离
- 🛠️ **Agent 自管理**：Agent 可通过 CLI 命令自主管理运行时（查看状态、切换模型、调整配置等）
- 📦 **项目搬家**：`evolclaw mv` 一键迁移项目目录，保留 Claude/Codex/EvolClaw 全部会话历史
- 💾 **会话与队列持久化**：会话数据与 CLI 工具共享，队列状态持久化存储，服务重启不丢失，支持 `/queue` 命令查看队列状态
- ⚡ **执行中插入**：任务执行中可发送新消息，自动中断当前任务并处理新请求
- 🔕 **消息智能发送**：前台任务动态聚合批量发送，后台任务静默完成后通知
- 🧩 **Agent 多实例**：一个 JSON 文件定义一个 Agent，多 Agent 并发运行，Agent 运行时隔离 + 热重载无需重启
- ⏰ **Trigger 自主任务**：Agent 可设置延迟/定时/周期任务，cron 表达式支持，独立 silent session 执行，支持脚本模式和 prompt 模式
- 🔧 **Daemon 模块**：系统级后台服务，管理升级检查、全局触发器等跨 Agent 任务
- 🎯 **Agent 启动引导**：首次启动自动引导配置 AID、owners、channels，降低上手门槛
- 🎭 **主动模式策略**：proactive 模式下智能判断响应时机，群聊支持静默策略，避免无效打扰
- 🎴 **交互卡片体系**：CommandCard + ActionInteraction，Feishu 与 AUN 统一支持
