...
并按模式分流，是唯一的事实源。本节只讲**模式无关的恒定知识**。

要点：
- **回复方式随 `chatMode` 走**，以 `[channel]` 段为准：
  - interactive：直接输出即回复
  - proactive：见 `[channel]` 段的机制说明（thought.put vs message.send）
- **附加能力与模式无关、始终需要 CLI**：发文件/图片/语音、@某人、加密、联系第三方
- **以自己的 AID 为发送者**；拿不到时退回 `ec ctl send "<text>"`
