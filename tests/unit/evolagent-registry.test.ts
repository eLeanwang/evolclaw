...
describe('EvolAgentRegistry', () => {
  ...
  it('handles missing agents directory gracefully', () => {
    ...
    const reg = new EvolAgentRegistry(missingDir);
  ...
  describe('isOwner / isAdmin / setChannelOwner routing', () => {
    ...
    it('isAdmin falls back for default channel', () => {
      const reg = new EvolAgentRegistry(agentsDir);
      reg.loadAll(configWithOwner());
      const cfg = configWithOwner();
      const fallback = (ch: string, uid: string) => {
        ...
        if (inst.owner === uid) return true;
        return Array.isArray(inst.admins) && inst.admins.includes(uid);
      };
      // owner of default channel is admin via fallback
      expect(reg.isAdmin('default-fs', 'def-owner', fallback)).toBe(true);
      // listed admin in fallback's source of truth
      expect(reg.isAdmin('default-fs', 'def-admin', fallback)).toBe(true);
      // unrelated user
      expect(reg.isAdmin('default-fs', 'someone-else', fallback)).toBe(false);
    });

    it('setShowActivities for default channel routes to globalWriter (not agent)', () => {
      const calls: Array<[string, string]> = [];
      const reg = new EvolAgentRegistry(agentsDir, {
        setOwner: () => {},
        setShowActivities: (ch, mode) => calls.push([ch, mode]),
      });
      reg.loadAll(configWithOwner());

      reg.setShowActivities('default-fs', 'dm-only');

      expect(calls).toEqual([['default-fs', 'dm-only']]);
    });

    it('setShowActivities default channel without globalWriter is a no-op (warns)', () => {
      const reg = new EvolAgentRegistry(agentsDir);  // no globalWriter wired
      reg.loadAll(configWithOwner());

      // Should not throw; warning is logged
      expect(() => reg.setShowActivities('default-fs', 'none')).not.toThrow();
    });

    it('setChannelOwner returns gracefully (warns) when channel not found anywhere', () => {
      const calls: Array<[string, string]> = [];
      const reg = new EvolAgentRegistry(agentsDir, {
        setOwner: (ch, uid) => calls.push([ch, uid]),
      });
      ...
      // Channel does not exist in any agent — no throw, no globalWriter call
      expect(() => reg.setChannelOwner('does-not-exist', 'whoever')).not.toThrow();
      expect(calls).toEqual([]);
    });

    it('setChannelOwner default channel without globalWriter is a no-op (warns)', () => {
      const reg = new EvolAgentRegistry(agentsDir);  // no globalWriter wired
      reg.loadAll(configWithOwner());

      // Default channel + no writer = warn-only, do not throw
      expect(() => reg.setChannelOwner('default-fs', 'whoever')).not.toThrow();
    });
  });
});
