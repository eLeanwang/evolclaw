...
function createMockAdapter(): ChannelAdapter & { sentMessages: string[] } {
  ...
  return {
    ...
    send: vi.fn().mockImplementation(async (_envelope: any, payload: any) => {
      if ('text' in payload) sentMessages.push(payload.text);
    }),
...
describe('Idle Timeout with StreamIdleMonitor', () => {
  ...
  it('should use default 120s when idleMonitor.timeout is not set', async () => {
    const runner = createMockAgentRunner([], 0);
    runner.runQuery.mockImplementation(async () => ({
      [Symbol.asyncIterator]() {
        return {
    ...
    // 不设置 idleMonitor.timeout → 默认 120000ms
    const config: Config = {
      agents: { anthropic: { apiKey: 'test' } },
      channels: {
        feishu: { appId: '', appSecret: '', owner: 'owner-123' },
